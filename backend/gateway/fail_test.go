...
func TestFailSpec(t *testing.T) {
	rows := []struct {
		...
		{"客户端取消", context.Canceled,
			failDecision{ErrKind: errKindClientCancel, Status: http.StatusBadGateway, RelayKind: relayErrAPI}},
		...
		{"已提交+断连", &streamCommittedError{err: io.ErrUnexpectedEOF},
			failDecision{BreakerFail: true, ErrKind: errKindStreamInterrupt, Status: http.StatusBadGateway, RelayKind: relayErrAPI}},
		...
		{"等头假死（快速开路+504）", &upstreamHeaderTimeoutError{seconds: 30},
			failDecision{Retryable: true, StallNoHeader: true, BreakerFail: true, ErrKind: errKindNetwork, Status: http.StatusGatewayTimeout, RelayKind: relayErrAPI}},
		{"流静默假死（保留重试+504）", &streamStallError{stage: "首包", wait: 30 * time.Second},
			failDecision{Retryable: true, StallMidStream: true, BreakerFail: true, ErrKind: errKindWatchdog, Status: http.StatusGatewayTimeout, RelayKind: relayErrAPI}},
		{"上游429+Retry-After透传", &upstreamStatusError{status: 429, retryAfter: "30"},
			failDecision{Retryable: true, BreakerFail: true, ErrKind: errKindUpstream, Status: http.StatusTooManyRequests, RelayKind: relayErrRateLimit, RetryAfter: "30"}},
		{"上游500", &upstreamStatusError{status: 500},
			failDecision{Retryable: true, BreakerFail: true, ErrKind: errKindUpstream, Status: 500, RelayKind: relayErrAPI}},
		{"上游503", &upstreamStatusError{status: 503},
			failDecision{Retryable: true, BreakerFail: true, ErrKind: errKindUpstream, Status: 503, RelayKind: relayErrAPI}},
		{"上游400（请求问题不重试不计熔断）", &upstreamStatusError{status: 400},
			failDecision{ErrKind: errKindUpstream, Status: 400, RelayKind: relayErrInvalidRequest}},
		{"上游401（渠道鉴权问题，引导换 key 而非改 payload）", &upstreamStatusError{status: 401},
			failDecision{ErrKind: errKindUpstream, Status: 401, RelayKind: relayErrAuth}},
		{"上游403", &upstreamStatusError{status: 403},
			failDecision{ErrKind: errKindUpstream, Status: 403, RelayKind: relayErrAuth}},
		{"上游404（目标不存在）", &upstreamStatusError{status: 404},
			failDecision{ErrKind: errKindUpstream, Status: 404, RelayKind: relayErrNotFound}},
		{"协议转换失败", &convertError{err: errors.New("解析请求失败")},
			failDecision{ErrKind: errKindConvert, Status: http.StatusBadGateway, RelayKind: relayErrAPI}},
		{"网络错误", fakeNetErr{},
			failDecision{Retryable: true, BreakerFail: true, ErrKind: errKindNetwork, Status: http.StatusBadGateway, RelayKind: relayErrAPI}},
		{"url.Error 包装非流式总超时（网络归因+504）", &url.Error{Op: "Post", Err: context.DeadlineExceeded},
			failDecision{Retryable: true, BreakerFail: true, ErrKind: errKindNetwork, Status: http.StatusGatewayTimeout, RelayKind: relayErrAPI}},
		{"裸 DeadlineExceeded（自满足 net.Error，仍 504）", context.DeadlineExceeded,
			failDecision{Retryable: true, BreakerFail: true, ErrKind: errKindNetwork, Status: http.StatusGatewayTimeout, RelayKind: relayErrAPI}},
		{"未知错误", errors.New("boom"),
			failDecision{ErrKind: errKindInternal, Status: http.StatusBadGateway, RelayKind: relayErrAPI}},
	}
